...
import { User, ChevronRight, Activity, MoveVertical, Weight, Flame } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { useLanguage } from '../context/LanguageContext';

export default function UserProfileModal({ isOpen, onClose, onComplete }) {
  ...
  return (
    ...
    >
      <div className="relative bg-[#07090f] border border-cyber-cyan/30 rounded-3xl w-full max-w-lg shadow-[0_0_50px_rgba(0,240,255,0.15)] flex flex-col max-h-[90vh]">
        
        {/* Close Button */}
        {onClose && (
          <button 
            onClick={onClose}
            className="absolute top-4 right-4 p-2 bg-black/50 hover:bg-white/10 text-gray-400 hover:text-white rounded-full transition-colors z-[100]"
          >
        ...
        {/* Header (Fixed) */}
        <div className="shrink-0 p-6 text-center space-y-3 bg-gradient-to-b from-cyber-cyan/10 to-transparent border-b border-cyber-cyan/20 relative">
          <div className="mx-auto w-14 h-14 bg-[#0a0f18] rounded-full flex items-center justify-center border border-cyber-cyan/40 shadow-[0_0_15px_rgba(0,240,255,0.3)] relative z-10">
        ...
        <div className="overflow-y-auto">
          <form onSubmit={handleSubmit} className="p-4 md:p-6 space-y-6">
            ...
            {/* Row 2: Height and Weight as Inputs */}
            <div className="grid grid-cols-2 gap-4">
              {/* Height Input */}
              <div className="space-y-1.5">
                <label className="text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center space-x-1.5">
