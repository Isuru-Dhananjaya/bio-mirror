  ...
  return (
    <div className="flex-1 min-w-[140px] bg-cyber-dark/80 backdrop-blur-md border border-cyber-border rounded-xl p-4 shadow-lg hover:border-cyber-cyan/50 transition-all duration-300 group">
      <div className="flex justify-between items-start">
        ...
        <span className={`text-[8px] px-2 py-0.5 rounded-full border font-bold tracking-widest ${theme.bg} ${theme.border} ${theme.text}`}>
          {status}
        </span>
      </div>
      
      <div className="mt-3 flex items-baseline space-x-1">
        <span className={`text-4xl md:text-5xl font-black font-mono tracking-tighter drop-shadow-[0_0_8px_currentColor] ${theme.text}`}>
          {value !== null ? value : '--'}
        </span>
